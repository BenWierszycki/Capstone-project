...
global_df = fetch_multiple_locations_data(locations_global)

# Check if dataframes are have data before merging
if uk_df is not None and global_df is not None:
    merged_df = pd.merge(uk_df, global_df, on='datetime', how='outer')
elif uk_df is not None:
    merged_df = uk_df
elif global_df is not None:
    merged_df = global_df
else:
    merged_df = pd.DataFrame(columns=['datetime'])
